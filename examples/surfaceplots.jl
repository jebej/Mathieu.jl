...
# ce₁(z,q)
figure(figsize=(10,8));
plot_surface(q,z,mapreduce(q->Mathieu.cea(0,q,z),hcat,q),cmap=ColorMap("coolwarm"))
title("ce₁(z,q)"); xlabel("q"); ylabel("z");
zlim([-1.2,1.2]); gca(projection="3d").view_init(azim=-30)

# se₁(z,q)
figure(figsize=(10,8));
plot_surface(q,z,mapreduce(q->Mathieu.sea(0,q,z),hcat,q),cmap=ColorMap("coolwarm"))
title("se₁(z,q)"); xlabel("q"); ylabel("z");
zlim([-1.5,1.5]); gca(projection="3d").view_init(azim=-30)

# se₂(z,q)
figure(figsize=(10,8));
plot_surface(q,z,mapreduce(q->Mathieu.sep(0,q,z),hcat,q),cmap=ColorMap("coolwarm"))
title("se₂(z,q)"); xlabel("q"); ylabel("z");
zlim([-1.2,1.2]); gca(projection="3d").view_init(azim=-30)

# ce₂(z,q)
figure(figsize=(10,8));
plot_surface(q,z,mapreduce(q->Mathieu.cep(1,q,z),hcat,q),cmap=ColorMap("coolwarm"))
title("ce₂(z,q)"); xlabel("q"); ylabel("z");
